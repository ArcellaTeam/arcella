...
use arcella_engine::ArcellaEngineResult;
...
pub trait ComponentItemSpecExt {
    ...
    fn to_spec(&self, engine: &Engine) -> ArcellaEngineResult<ComponentItemSpec>;
}
...
pub trait ComponentTypeExt {
    ...
    fn imports_spec(&self, engine: &Engine) -> ArcellaEngineResult<HashMap<String, ComponentItemSpec>>;

    /// Extracts exports as a map of `ComponentItemSpec`.
    fn exports_spec(&self, engine: &Engine) -> ArcellaEngineResult<HashMap<String, ComponentItemSpec>>;
}
...
fn to_spec_with_depth(
    ...
    depth: usize,
    max_depth: usize,
) -> ArcellaEngineResult<ComponentItemSpec> {
    if depth > max_depth {
...
mod tests {
    ...
    #[test]
    fn test_simple_component_func() -> ArcellaEngineResult<()> {
        ...
        let component = Component::new(&engine, wat)
            .map_err(|e| ArcellaEngineError::EngineError(format!("Failed to load .wasm: {}", e)))?;
    ...
    #[test]
    fn test_recursion_limit() -> ArcellaEngineResult<()> {
        let engine = Engine::default();
        let wat = r#"(component)"#;
        let component = Component::new(&engine, wat)
            .map_err(|e| ArcellaEngineError::EngineError(format!("Failed to load .wasm: {}", e)))?;
